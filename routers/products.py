...
from schemas.products import ProductResponse, PagedProducts, CreateProduct
...
async def create_product(product: CreateProduct, db: AsyncSession = Depends(get_connection)):
    new_product = Product(
        name=product.name,
...
async def get_products(limit: int = 50, offset: int = 0, db: AsyncSession = Depends(get_connection)):
    ...
    products_result = await db.execute(select(Product).offset(offset * limit).limit(limit))
